...
# CI/nightly 门禁：任意非 skipped 行比 HF 慢 10% 以上即失败，并保存 JSON
...
MoonBit 语料定义在 `src/benchmarks/bench_test.mbt`，Python 基线在
...
`--fail-above <ratio>` 会把同一份对比结果变成回归门禁：任何非 skipped 行超过阈值
就以非 0 状态退出；`--json-out` 会把完整 rows、skipped HF baselines、optimization
focus 与 failures 写成机器可读 JSON，方便 CI artifact 或 nightly 趋势收集。
...
- **正确性优先。** 40 个真实模型（fixture 存在时）与 Python `tokenizers` 做
  token id 对拍。
