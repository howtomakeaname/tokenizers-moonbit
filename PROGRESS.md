...
| P3 | Python binding 兼容层 / 命名别名 | 🚧 state/命名别名/错误分类第三十五批已完成 | 已覆盖 tokenizer/model/component/encoding/trainer 的 state、tuple/buffer、属性 getter、显示 alias、构造器 alias、sequence/item 访问等 Python binding 迁移入口；详细分批记录见下方第三十批及之后段落，后续继续补低频属性与方法 alias，MoonBit 原生 API 保持 typed/显式风格 | 独立里程碑 |
