...
| P2 | 通用 Split/Replace 正则覆盖 | 🚧 | Normalizer/Decoder Replace 已支持 `^\\s+` / `\\s+` / `\\s+$` / `\\s{2,}` / `[\\r\\n]+` / `[^\\S\\r\\n]+` / `[^\\S\\r\\n]{2,}` / ` {2,}` / anchored digit/word/letter/punctuation/symbol runs / `\\d+` / `[\\d]+` / `\\D+` / `\\P{N}+` / `\\d{2,}` / `[\\d]{2,}` / `\\d{2}` / `\\d{3}` / `\\d{4}` / `\\d{1,n}` 常见 bounded digit / `\\w{2,}` / `\\w{2}` / `[A-Za-z0-9]{3}` / `[A-Za-z]{2,}` / `\\p{L}{3}` / `\\p{P}{2,}` / `\\p{P}{2}` / `\\p{S}{2}` / `\\w+` / `\\W+` / `[A-Za-z0-9]+` / `[^A-Za-z0-9]+` / `[A-Za-z]+` / `[^A-Za-z]+` / `\\p{L}+` / `\\P{L}+` / `\\p{P}+` / `\\P{P}+` / `\\p{S}+` / `\\P{S}+` / `[\\p{P}\\p{S}]+` / `[^\\s\\p{L}\\p{N}]+` 与常见 ASCII/Unicode word/letter/number/punctuation/symbol class；Split 已补 literal / `^\\s+` / `\\s+` / `\\S+` / `\\s+$` / `\\s{2,}` / `[\\r\\n]+` / `[^\\S\\r\\n]+` / anchored digit/word/letter/punctuation/symbol runs / `\\d+` / `[\\d]+` / `\\D+` / `\\P{N}+` / `\\d{2,}` / `[\\d]{2,}` / `\\d{2}` / `\\d{3}` / `\\d{4}` / `\\d{1,2}` / `\\d{1,3}` / `\\d{1,4}` / `\\p{N}{1,2}` / `\\p{N}{1,3}` / `\\p{N}{1,4}` / `\\w{2,}` / `\\w{2}` / `[A-Za-z0-9]{3}` / `[A-Za-z]{2,}` / `\\p{L}{3}` / `\\p{P}{2,}` / `\\p{P}{2}` / `\\p{S}{2}` / `\\w+` / `\\W+` / `[A-Za-z0-9]+` / `[^A-Za-z0-9]+` / `[A-Za-z]+` / `[^A-Za-z]+` / `\\p{L}+` / `\\P{L}+` / `\\p{P}+` / `\\P{P}+` / `\\p{S}+` / `\\P{S}+` / `[\\p{P}\\p{S}]+` / `[^\\s\\p{L}\\p{N}]+` 的行为与 offset 测试；复杂未知 pattern 加载期显式 Unsupported |
