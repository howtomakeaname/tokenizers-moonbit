...
Array 增长分配；`from_pretrained` 增加单项 source cache 后，quick native 抽样中
gpt2/Qwen2.5/qwen3_coder from_str 分别约 0.82x/0.83x/0.82x，llama from_str 同档
约 0.99x，local from_pretrained-file 已全部进入同档或快于 HF（llama 约 1.02x）。
下一轮性能优化优先级：完整 `--corpus all` 矩阵复测 > 大词表 JSON 解析。
...
3. **训练 / Hub 集成**：当前定位 inference-first；`to_json`/`save` 已支持原始 JSON 往返，`from_pretrained` 已支持本地目录/文件与稳定路径 source cache。训练器和 Hub 网络下载 API 暂未实现。
