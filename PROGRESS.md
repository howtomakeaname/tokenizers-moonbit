...
Array 增长分配；quick native 抽样中 gpt2/Qwen2.5/qwen3_coder from_str 分别约
0.76x/0.73x/0.77x，llama from_str 回到同档约 0.92x。下一轮性能优化优先级：
完整 `--corpus all` 矩阵复测 > llama/from_pretrained 文件读取路径 > 大词表 JSON 解析。
