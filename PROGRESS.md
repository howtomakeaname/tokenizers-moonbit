...
| P2 | 通用 Split/Replace 正则覆盖 | 🚧 | Normalizer/Decoder Replace 已支持 `\\s+` / `^\\s+` / `\\s+$` / `[\\r\\n]+` / `[^\\S\\r\\n]+` / ` {2,}` / `\\d+` / `\\D+` / `\\w+` / `\\W+` / `\\p{L}+` / `\\P{L}+` 与常见 ASCII/Unicode word/letter class；Split 已补 literal / `\\s+` / `\\S+` / `\\s+$` / `[\\r\\n]` / `\\d+` / `\\D+` / `\\w+` / `\\W+` / `\\p{L}+` / `\\P{L}+` / digit triplet 的行为与 offset 测试；复杂未知 pattern 加载期显式 Unsupported |
...
2. **正则**：core 正则不提供完整通用 Unicode regex 引擎；GPT/Qwen/o200k 主线已手写扫描器。通用 Split 已补 literal、`\\s+`、`\\S+`、`\\s+$`、`[\\r\\n]`、`\\d+`、`\\D+`、`\\w+`、`\\W+`、`\\p{L}+`、`\\P{L}+`、常见 ASCII word class、digit triplet；复杂未知 pattern 加载期显式 Unsupported，避免静默不对齐。Replace normalizer/decoder 已补 `\\s+` / `^\\s+` / `\\s+$` / `[\\r\\n]+` / `[^\\S\\r\\n]+` / ` {2,}` / `\\d+` / `\\D+` / `\\w+` / `\\W+` / `\\p{L}+` / `\\P{L}+`；更复杂 Replace pattern 仍按字面量处理。
