...
| P2 | 通用 Split/Replace 正则覆盖 | 🚧 | Normalizer/Decoder Replace 已支持 `^\\s+` / `\\s+` / `\\s+$` / `\\s{2,}` / `[\\r\\n]+` / `[^\\S\\r\\n]+` / `[^\\S\\r\\n]{2,}` / ` {2,}` / anchored digit/word/letter/punctuation/symbol runs / `\\d+` / `\\D+` / `\\d{1,n}` 常见 bounded digit / `\\w+` / `\\W+` / `[A-Za-z]+` / `[^A-Za-z]+` / `\\p{L}+` / `\\P{L}+` / `\\p{P}+` / `\\P{P}+` / `\\p{S}+` / `\\P{S}+` / `[\\p{P}\\p{S}]+` / `[^\\s\\p{L}\\p{N}]+` 与常见 ASCII/Unicode word/letter/number/punctuation/symbol class；Split 已补 literal / `^\\s+` / `\\s+` / `\\S+` / `\\s+$` / `\\s{2,}` / `[\\r\\n]+` / `[^\\S\\r\\n]+` / anchored digit/word/letter/punctuation/symbol runs / `\\d{1,2}` / `\\d{1,3}` / `\\d{1,4}` / `\\p{N}{1,2}` / `\\p{N}{1,3}` / `\\p{N}{1,4}` / `\\w+` / `\\W+` / `[A-Za-z]+` / `[^A-Za-z]+` / `\\p{L}+` / `\\P{L}+` / `\\p{P}+` / `\\P{P}+` / `\\p{S}+` / `\\P{S}+` / `[\\p{P}\\p{S}]+` / `[^\\s\\p{L}\\p{N}]+` 的行为与 offset 测试；复杂未知 pattern 加载期显式 Unsupported |
...
2. **正则**：core 正则不提供完整通用 Unicode regex 引擎；GPT/Qwen/o200k 主线已手写扫描器。通用 Split 已补 literal、`^\\s+`、`\\s+`、`\\S+`、`\\s+$`、`\\s{2,}`、`[\\r\\n]+`、`[^\\S\\r\\n]+`、`\\d+`、`\\D+`、`\\d{1,2}`、`\\d{1,3}`、`\\d{1,4}`、`\\p{N}{1,2}`、`\\p{N}{1,3}`、`\\p{N}{1,4}`、anchored digit/word/letter/punctuation/symbol runs、`\\w+`、`\\W+`、`[A-Za-z]+`、`[^A-Za-z]+`、`\\p{L}+`、`\\P{L}+`、`\\p{P}+`、`\\P{P}+`、`\\p{S}+`、`\\P{S}+`、`[\\p{P}\\p{S}]+`、`[^\\s\\p{L}\\p{N}]+`、常见 ASCII/Unicode word/letter/number/punctuation/symbol class；复杂未知 pattern 加载期显式 Unsupported，避免静默不对齐。Replace normalizer/decoder 已补 `^\\s+` / `\\s+` / `\\s+$` / `\\s{2,}` / `[\\r\\n]+` / `[^\\S\\r\\n]+` / `[^\\S\\r\\n]{2,}` / ` {2,}` / anchored digit/word/letter/punctuation/symbol runs / `\\d+` / `\\D+` / 常见 bounded digit / `\\w+` / `\\W+` / `[A-Za-z]+` / `[^A-Za-z]+` / `\\p{L}+` / `\\P{L}+` / `\\p{P}+` / `\\P{P}+` / `\\p{S}+` / `\\P{S}+` / `[\\p{P}\\p{S}]+` / `[^\\s\\p{L}\\p{N}]+`；更复杂 Replace pattern 仍按字面量处理。
