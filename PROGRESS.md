...
| R11 | HF tokenizers 全量兼容缺口收敛（低频 API / 高级训练 / 正则与并行） | 🚧 排期中（`encode_fast`、迁移序列化 API、缓存控制 API、BPE dropout、模型文件级保存、HF config alias、EncodeInput、Token/Regex/Encoding/Tokenizer state API、Tokenizer train pipeline MVP、高级 trainer knobs 第一批、Hub sidecar/metadata/离线校验/诊断 hint、batch parallel 兼容入口、NormalizedString/PreTokenizedString、async 兼容入口、regex literal 策略、错误分类 helper 已完成） |
...
| P3 | 完整 Hub 生态 | 🚧 tokenizer.json 下载 + sidecar/metadata/离线校验/诊断已完成 | 已补 `PretrainedAuxFiles` 与 `from_pretrained_aux_files`，可按本地目录/文件/Hub cache snapshot 读取 `tokenizer_config.json` / `special_tokens_map.json`；`from_pretrained_downloaded` 可同步写入两个 sidecar 与 `tokenizer.json.etag`，新增 `PretrainedCacheMetadata` / `from_pretrained_cache_metadata` 返回 tokenizer/sidecar 路径、revision、resolved revision、etag，并提供 `cache_exists` / `etag_matches` / `is_fresh` 与 tokenizer 级 helper 做离线 freshness 校验；新增 `PretrainedResolutionHint` / `from_pretrained_resolution_hint`，缺失 cache/snapshot/tokenizer.json 时输出含 private/gated repo 指引的可诊断消息。后续继续补 HEAD/断点缓存策略；保持请求 header 与 HF 兼容，不引入非 HF 标识 | 1–2 周 |
| P3 | Python binding 兼容层 / 命名别名 | 🚧 state/命名别名/错误分类第二批已完成 | 已提供 `Tokenizer::to_str(pretty)` / `save(pretty)` 命名 alias、`Encoding::__getstate__` / `__setstate__`、`TokenizerState` / `Tokenizer::get_state` / `from_state` / `__getstate__` / `__setstate__`，覆盖 Python pickle/state 与 JSON 状态迁移；新增 `EncodingBuffers` / `Encoding::from_buffers` / `get_buffers` / `as_buffers` / `__len__` / `get_n_sequences`、`TokenState` / `Token::__getstate__` / `__setstate__`、`TokenizerErrorKind` / `TokenizerError::kind` / `message`，方便 Python binding 做 buffer/tuple/state 与异常类型映射；后续继续补更多方法/属性 alias；MoonBit 原生 API 不因此变复杂 | 独立里程碑 |
