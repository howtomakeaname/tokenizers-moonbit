...
- 发布：mooncakes `howtomakeaname/tokenizers-moonbit`，已发 0.1.0→**0.15.1**（2026-09-26，0.15.0 后含 PR #65 锚定字母形 + 星号/双可选尾）。0.15.1 后待办见 §2 队列。
...
### 最近工作日志（新在上）
- **2026-09-26 发版 0.15.1（已发布，干净验证 4/4×三后端）**：0.15.0 后 PR #65 单批。直接推 main：bump + `moon publish` 200 OK + /tmp/verify0151 干净验证（`^\q{2}$`、`[\o]`、`\q*`、`a??`——四代表行为 token 级与 HF 一致）。
- **2026-09-26 PR #65**（6 commit，三轮评审，round-3 APPROVE；电池 1428+336+400 单元）：**锚定字母形 + 星号/双可选尾小缺口关闭**。三个锚定族（行锚/\b\B 窗/双锚整行）的 escape map 委托 letter helper——`^\q{2}$`/`\b\q{2}`/`\B\q{2}` 等全形计算；`[\o]` 非大括号=字面量 o（八进制形调用点守卫保持响亮）；`\X*` 三字母星尾改写裸 `{0,}` 走查（含 o——八进制不能以 * 开头）；双可选族补全（`a??` n==3、`\q??` n==4、`[a\w]??` 整括号 → 全空 `{0}`）。**评审三轮战果**：R1 抓 n>=3 前缀静默丢弃（`ab??` HF 解析为前缀+惰性可选，68 单元）→ n==3；R2 抓修复误删反斜杠守卫（`\??` 是可选字面量 ?，HF 计算，被静默认领 no-op）→ 守卫恢复。遗留响亮（记录）：`ab?`/`ab??` 多字符+可选族、`\??`、八进制类 `[^\o{2}]`/`^\o{2}$`、`\x*`、`a*`/`\w*`/`\w??`/`\q*?`/`\q**`/`a???`、`\p`/`\P`/`\u` 裸形。四后端 525/502 绿；金标 429 单元 + 评审独立电池；CI 8/8。R11 两项（trainer EM/Hub sidecar）按用户未答复默认搁置。
- **2026-09-25 发版 0.15.0（已发布，干净验证 3/3×三后端）**：0.14.1 后 PR #64 单批。直接推 main：bump + `moon publish` 200 OK + /tmp/verify0150 干净验证（`\q{0}` 全空走查、`\o+` 字面量 o、`a?` 可选尾——三代表行为 token 级与 HF 一致）。
- **2026-09-25 PR #64**（7 commit，三轮评审，round-3 APPROVE，72×43 网格电池 2156 计算单元零错零抢跑）：**未知转义字面量族 + 通用可选 `?` 尾**。fresh 对象全 ASCII 字母扫描分类：15 个转义字母=字母本身（g i j k l m q E F I J L Q T U V），4 个=控制字符（`\a` BEL、`\e` ESC、`\f` FF、`\v` VT）；共享 `escaped_letter_literal` helper 接入双 zero_min map、char_bounded 双 map、括号类、裸字面量路径七处。**`'o'` 是 onig 八进制转义**（`\o{101}`≡'A'、`\o{0}`≡NUL、逗号/区间大括号 HF 构造错误——R1 抓到的静默错）：从 helper 移除，字面量 o 仅存于 `\o+` plus map 与裸 `\o`/`\o?`/`\o{,m}`（全数字上界守卫——R2 抓的 `\o{,}` 字面量漏网）。`\x` 为怪癖（裸=字面 x、量词形永不匹配）保持响亮。**通用 `?` 尾**≡`{0,1}` 文本改写（任意基；守卫在 `?` 本身的转义与前置量词符）。评审另确认：断言字母（b z A G K Y Z y）量词形 HF 报错=本库响亮一致；`\p`/`\P`/`\u` 裸形 HF 可算（响亮记录）；`\c` 永不匹配；`\h` 十六进制类。遗留响亮（记录）：`[\o]`、`a\o`、`\o*`、锚定字母形、`a??`/`ab?` 等。四后端 524/501 绿；金标 2100 单元 + 评审 2244/2156 单元；CI 8/8。
- **2026-09-25 发版 0.14.1（已发布，干净验证 3/3×三后端）**：0.14.0 后 PR #63 单批。直接推 main：bump + `moon publish` 200 OK + /tmp/verify0141 干净验证（`\+{0}` 转义加号、`a++{0}` 占有零大括号、`a+{1,0}` 逆序 kin——三代表行为 token 级与 HF 一致）。
- **2026-09-25 PR #63**（8 commit，四轮评审，round-4 APPROVE；B/C 33+D 170 单元电池零失配）：**Python binding 别名审计（已尽）+ 最后响亮缺口关闭**。审计：枚举 HF 0.22.2 全组件公开 API（263 方法，Tokenizer/Encoding/AddedToken/Regex/normalizer/pre_tokenizer/decoder/processor/model/trainer），全部在库内有对应（30 个 `custom` 为 PyO3 dir() 工件；3 个 `read_file` 已有 `Model::*_read_file`）——R11 别名长尾就此关闭。缺口关闭：转义加号字面量基 `'+'` 入三处 escape map（`\+{0}` 全空走查、`\+{2}` 贪婪、`\+??+`/`\+*+` 经既有规则折叠）；digit-triplet 扫描器只把纯数字块标为匹配（`\p{N}{1,3}` 留有 regex gap——探针 `Split(removed)` 于 'abc12345de'→['abc','de']，38/170 main 分歧 cell 全修）；**占有+大括号值分类**（`{0}`/`{00}` 全空、`{0,0}` 全空、`{0,m}`/`{0,}` 星走查、`{1}`/`{01}`/`{1,m}` 贪婪、n≥2 永不匹配=响亮、畸形尾/超界=字面量/错误）；**逆序界全家四分类**（up==0 星走查、up==1 贪婪、up≥2 占有=响亮/kin=`X{up,}` 窗口）——其中 **kin `a+{1,0}` 是 main 上的静默错**（'b#b' vs HF '#b#b#'，round-2 发现）。探针方法论：共享 Replace 对象的一次读数异常（`\w{2,2}?+` 误读为可选组），fresh-object 三次重试纠正为链族——一律 fresh 对象探针。剩余响亮缺口（记录）：`\q{0}` 未知转义（HF 计算）、`a++{n≥2}` 永不匹配接受形、区间 `?+` 链族。四后端 523/500 绿；CI 8/8。R11 余项：trainer EM（需外部数据集）、Hub sidecar——待用户拍板。
- **2026-09-24 发版 0.14.0（已发布，干净验证 3/3×三后端）**：0.13.1 后 PR #62 单批。直接推 main：bump + `moon publish` 200 OK + /tmp/verify0140 干净验证（gpt2-pattern Split Removed→0 token、CLIP 真实配置 Removed+invert 只留匹配、Isolated 逐段——三代表行为 token 级与 HF 一致；验证 harness 两处自身 bug 在过程中被定位修复：JSON pattern 对象少一个 `}`、bisect 补丁忘还原——库本身无问题）。
...
- **2026-09-24 PR #60**（8 commit，三轮评审，round-3 APPROVE，~3,100 对拍单元）：**占有量词/双尾部折叠 + 集合惰性 n 组 + 裸点窗**。lazy_normalize 新增（均带转义守卫）：`X++`≡贪婪 `X+`（含锚定 `\ba++`，尾锚回缩形 `\ba++\B` 保持拒绝）、`X+*`≡`X{0,}`、`X+{n}/{n,m}/{n,}`≡`X{n,}`（取首数；`X++{n}` 占有+大括号 onig 永不匹配→拒绝）、`X?+`≡`{0,1}`、`X*+`≡`{0,}`；大括号双惰性：精确 `{n}??`≡全空 `{0}`、区间 `{n,m}??`≡可选组 `{n}?`（'aaa'→'#a#' 尾空匹配区分于贪婪——金标 sweep 自纠）、**逆序界 `{3,2}??`≡onig 交换后全空**；集合 `[a\w]{n}?` 惰性组走共享 lazy_ranged_or_empty（5 元组 lazy_group 标志，bracket_set_spec 公开签名变更→.mbti 需随提交，CI Release gates 强制）；class_bounded 认领裸 `.+`/`.{n,m}`（点基 kind 30，1 字符基放宽 open>=1）。**评审三轮战果**：R1 抓畸形大括号尾（`{2,3x}??` 被 HF 字面量语义、我们误改写）与占有+大括号误折叠；R2 抓逆序双惰性界；并发现 **main 上既有 DoS 级挂死**——`{n,0}?` 经 lazy_exact 逆序交换得 lo=0，引擎空匹配永不前进（恶意 tokenizer.json 可挂死 normalize）——lo<=0 委派 `{0,n}` 走查修复且行为与 HF 一致（'aaa'→'##'）。遗留响亮缺口（记录）：`a+{0}`/`[a\w]{0}?`/`\w{,3}??`/点零下限窗 `.{0,2}`/死中间体（`a+*+` 等）/集合逆序单惰性 `[a\w]{2,0}?`。四后端 519/496 绿；金标 360 单元 + 评审独立 516/564 单元零分歧；CI 8/8。
- **2026-09-24 发版 0.12.1（已发布，干净验证 6/6×三后端）**：0.12.0 后 PR #59 单批。直接推 main：bump + `moon publish` 200 OK + /tmp/verify0121 干净验证（`{,2}`、`{2,0}`、`+??`、`a+`、`a+?`、`{2,1}` 回归——六代表行为 token 级与 HF 一致）。
- **2026-09-24 PR #59**（4 commit，两轮评审，round-2 APPROVE）：**声明性缺口关闭**（PR #58 评审 pin 的响亮拒绝逐项落地）：集合大括号 comma-open `[a\w]{,2}`≡`{0,2}`、`{n,0}` 换 `\equiv{0,n}` 空匹配走查；`{1}?`≡可选单字符组≡`{0,1}`——`+??` 改写产物 `[a\w]{1}?` 由此落地（链路终点此前断在 lazy_exact 只认 kind/字面量基）；裸字面量 plus 入 char_bounded 族（`a+`≡`a{1,}`、`a+?`≡`a{1}`，转义基 `\n+`/`\.+` 等含）。**评审 round-1 抓真回归**：`{n,0}` 分支误把非零逆序界一并改写零下限（`{2,1}` 应保窗口交换 `{1,2}`，475 单元中 18 发散）——修为仅 `w==0` 走空匹配走查并加回归 pin（此前无测试覆盖非惰性逆序集合界）。剩余相邻响亮缺口（记录）：占有量 `a++`/`a+*`/`a+{2}`、`[a\w]{1}??`/`{11}?`、`{,}`/`{,2x}` 字面量形式。四后端 519/496 绿；金标 187 单元首轮零分歧 + 评审 475/560 单元（修复后零分歧）；CI 8/8。
- **2026-09-24 发版 0.12.0（已发布，干净验证 5/5×三后端）**：0.11.1 后 PR #58 单批。直接推 main：bump + `moon publish` 200 OK + /tmp/verify0120 干净验证（`[a\w]+`、`[^a]+`、`[a-c]{2,4}`、`\w+??`、`[\p{L}\d]` 五代表行为 token 级与 HF 一致）。
- **2026-09-24 PR #58**（6 commit，两轮评审，round-2 APPROVE）：**通用括号字符类族** `[members]`/`[^members]`（字面量/字面量区间/转义字面量/类上下文快捷类——类内 `\w` 为无怪癖 kind 26，探针 `[a\w]` 于 'a²b'→'#²#'；`^` 补集含 `\n`；首尾 `-` 为字面量；逆序区间与快捷类区间端点 = onig 加载错误显式拒绝；单快捷类体归 canonical 拼写）。量词：无=(1,1) 逐字符、`+`=(1,-1)、大括号（逆序交换）、惰性 `+?`=(1,1)、零下限尾（`{0}`/`{0,m}`/`{0,}`/`?`/`*`）走空匹配走查引擎。**双尾部归一**（lazy_normalize 循环化）：无锚 `X+??` = 可选惰性组 `(X+?)?`——**pred 失败位空匹配回退**（`\w+??` 于 '-ab'→'#-##'，pred-hold 输入无法区分该语义，本轮评审方法论点）→ 改写 `X{1}?` 与 `{n}?` 折叠复合到 `X{0,1}`；`X+?+` 折叠贪婪 plus；裸类 `X+?` 折叠未量词类。**评审三轮抓真问题**：raw `&&`（onig 交集）被并集解析静默错（39 单元）→ 显式拒绝；`\p{Name}` 成员拼写漏写 `{`（八个 `\p` 行死代码）→ 修复；load 门在归一后漏查 lazy_exact（引擎算得对、门拒绝）→ 补查（连带认领 `a+??`）。声明性响亮缺口已 pin：`{,m}`/`{n,0}` 集合形式、括号基 `+??` 产物、`a+`/`a+?` 裸字面量 plus、锚定 `+??`。四后端 519/496 绿；金标 840+36 + 评审独立 810+54+338 单元零分歧；CI 8/8。
- **2026-09-24 发版 0.11.1（已发布，干净验证 5/5×三后端）**：0.11.0 后 PR #57 单批。直接推 main：bump + `moon publish` 200 OK + /tmp/verify0111 干净验证（`^.{2}`、`.{2}$`、`^.+`、`^.+?`、`.+?$` 五代表行为与 HF 逐字符一致）。
- **2026-09-24 PR #57**（2 commit，一轮评审 APPROVE）：**点基单锚窗口**。`single_anchor_window_spec` 接受单点基（kind 30，`.`≠\n）+ 大括号窗（`^.{2}`/`.{2}$`/`{n,m}`/`{n,}`，`\r` 留在行内：`aa\r\nbb` 于 `.{2}$`→`a#\n#`）；plus 拼写按锚定惰性规则（`^.+`≡`^.{1,}`、`^.+?`≡`^.{1}` 每行首一字符、**`.+?$`≡`.{1,}$`**——行尾锚迫使惰性升到行尾，与 \b 尾锚规则同构，基类无关含 `^\w+?`）；裸全点序列 = 精确窗（`^..`≡`^.{2}`、`^.`≡`^.{1}`）；量化多点仍拒（量词只绑末点）。零下限维持显式拒绝（探针：空行空匹配走查 + `$` 匹 EOS——另一种引擎）；裸 `.`（无锚每字符替换）范围外。评审独立对拍 684 normalizer + 156 decoder 单元零分歧、109 拼写 spec 元组 vs main 字节等价、被遮蔽的锚定类 run 族 285 单元双树一致。遗留（评审记录，非阻塞）：`+??`≡`+?`、`^.+?+`≡贪婪 `.+` 两个响亮拒绝覆盖缺口。四后端 518/495 全绿 + CI 8/8（含 HF benchmark）。注：/tmp parity-driver 与 case 数据被会话清理，本轮实模验证走库内 fixture 套件 + CI 双 smoke。
