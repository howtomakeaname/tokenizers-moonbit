...
Plume 文档站 locales / Mermaid 样式小闭环：文档站已按 Plume locales 规范把英文与中文 navbar/sidebar 拆到 theme locale 配置，不再在英文导航里手写“中文”入口；Mermaid 图表通过 `@vuepress/plugin-markdown-chart/client` 的 `defineMermaidConfig` 配置非紫色主题变量，并补 `.mermaid-wrapper` / `.mermaid-content` CSS 兜底，保持 pipeline / migration / benchmark / docs workflow 图在 light/dark 模式下统一使用绿色、青色与中性色系。
