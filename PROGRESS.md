...
| P2 | 通用 Split/Replace 正则覆盖 | 🚧 | Normalizer/Decoder Replace 已支持 `\\s+` / ` {2,}`；Split 已补 literal / `\\s+` / `\\S+` / `\\s+$` / `[\\r\\n]` / digit triplet 的行为与 offset 测试；复杂未知 pattern 加载期显式 Unsupported |
...
| P4 | trainer / training API | 🚧 | 已提供确定性 WordLevel trainer MVP（WhitespaceSplit + min_frequency + special tokens）、训练结果可 `to_json`/`save` 往返，并加入 HF trainer bench；BPE/WordPiece/Unigram trainer 待排期 |
...
2. **正则**：core 正则不支持 `\p{L}`；GPT/Qwen/o200k 主线已手写扫描器。通用 Split 已补 literal、`\\s+`、`\\S+`、`\\s+$`、`[\\r\\n]`、digit triplet；复杂未知 pattern 加载期显式 Unsupported，避免静默不对齐。Replace normalizer/decoder 已补 `\\s+` / ` {2,}`；更复杂 Replace pattern 仍按字面量处理。
