...
Hub sidecar request plan/cache 小闭环：新增 `hub_file_url` / `plan_hub_file_request`，复用 tokenizer.json 的 endpoint/revision 编码与 HF-style headers，支持 tokenizer 邻近 basename sidecar 的本地纯规划；`apply_hub_file_download_result` 可把完整 sidecar response body 写为 raw snapshot 文件；非 tokenizer.json 文件仍不附带 tokenizer cache/range metadata，也不做 sidecar 自动下载、ETag/Range/resume 或内容解析。
