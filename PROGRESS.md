...
### 2026-07-12 小闭环：trainer.mbt 模块化拆分

- 拆分 `trainer.mbt`（1133 行）为：
...
- 本版累计内容：truncation overflow 单序列全链路修复（PR #1）、pair overflow 0.22.x 窗口叉积（PR #3）、文档可运行化（PR #2）、stride 配置期校验（PR #4）、CI 工具链漂移修复（StringBuilder / Map 字面量 / formatter / .mbti 再生成 + tokenizer 包真 unused `core/bench` 导入清理——20260904 工具链收紧 unused_package 诊断）。
...
- 效果（行为对比扫描）：合成 **857 → 971/1008**（offsets 失败 151 → 约 20）、真实模型 **59 → 94/100**（41 → 6）。gpt2/llama/qwen/bert 全绿，t5 剩 5 例 charsmap+Metaspace 复合偏移、qwen3 1 例。
