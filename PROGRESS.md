...
训练默认值剩余风险：HF 0.22.x 的 WordLevel/WordPiece/BPE `min_frequency` 默认是 `0`，MoonBit 已对齐 WordLevel/WordPiece，BPE 仍保留历史默认 `2` 并有基线测试；BPE/Unigram 默认 unk 插入策略也仍与 HF 默认存在差异。这两类会直接改变训练产物和 unknown encode 行为，后续需独立 parity 步骤与 golden 对拍，不混入普通 API alias 补齐。
